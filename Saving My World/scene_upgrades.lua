...
local button_upgrade1, button_upgrade2, button_menu, sceneTitle, warningMessage
-- -------------------------------------------------------------------------------
...
local function onUpgrade1Touch(event) 
    if (event.phase == "ended") then
        ...
        if (user.money >= (user.shootlevel * _SHOOTUPGRADECOST + _SHOOTUPGRADECOST)) then
            ...
            if (user.shootlevel >= _SHOOTLEVELMAX) then
                -- display max level
                button_upgrade1:setLabel("Max Level")
            else
                ...
                user = loadsave.loadTable("user.json")
                button_upgrade1:setLabel("$"..(user.shootlevel * _SHOOTUPGRADECOST + _SHOOTUPGRADECOST).." Rank "..user.shootlevel)
            end
...
local function onUpgrade2Touch(event) 
    if (event.phase == "ended") then
        ...
        if (user.money >= (user.liveslevel * _LIVESUPGRADECOST + _LIVESUPGRADECOST)) then
            ...
            if (user.liveslevel >= _LIVESLEVELMAX) then
                -- display max level
                button_upgrade2:setLabel("Max Level")
            else
                -- proceed with upgrade
                ...
                loadsave.saveTable(user, "user.json")
                user = loadsave.loadTable("user.json")
                button_upgrade2:setLabel("$"..(user.liveslevel * _LIVESUPGRADECOST + _LIVESUPGRADECOST).." Rank "..user.liveslevel)
            end
...
function scene:create( event )
    ...
    -- adding buttons
    button_upgrade1 = widget.newButton {
        width = 480,
        height = 183,
    ...
    button_upgrade1.x = _CX - button_upgrade1.width * 0.6
    button_upgrade1.y = _CY
    sceneGroup:insert(button_upgrade1)


    button_upgrade2 = widget.newButton {
        width = 480,
        height = 183,
    ...
    button_upgrade2.x = _CX + button_upgrade2.width * 0.6
    button_upgrade2.y = _CY
    sceneGroup:insert(button_upgrade2)


    button_menu = widget.newButton {
        width = 426,
        height = 183,
    ...
    button_menu.x = _CX
    button_menu.y = button_upgrade1.y + 260
